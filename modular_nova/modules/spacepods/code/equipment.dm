// MODULE ID: SPACEPODS
// Modular spacepod equipment: weapons, cargo, seats, trackers, and locks.
// Ported from Whitesands (whitesands/code/modules/spacepods/equipment.dm).

/obj/item/spacepod_equipment
	name = "pod equipment"
	icon = 'modular_nova/modules/spacepods/icons/equipment.dmi'
	var/obj/spacepod/spacepod
	var/slot = SPACEPOD_SLOT_MISC
	var/slot_space = 1

/obj/item/spacepod_equipment/Destroy()
	if(spacepod && (src in spacepod.equipment))
		on_uninstall()
	return ..()

/obj/item/spacepod_equipment/proc/on_install(obj/spacepod/installed_pod)
	spacepod = installed_pod
	installed_pod.equipment |= src
	forceMove(installed_pod)

/obj/item/spacepod_equipment/proc/on_uninstall()
	if(spacepod)
		spacepod.equipment -= src
	spacepod = null

/obj/item/spacepod_equipment/proc/can_install(obj/spacepod/installed_pod, mob/user)
	var/room = installed_pod.equipment_slot_limits[slot] || 0
	for(var/obj/item/spacepod_equipment/equip in installed_pod.equipment)
		if(equip.slot == slot)
			room -= equip.slot_space
	if(room < slot_space)
		to_chat(user, span_warning("There's no room for another [slot] system!"))
		return FALSE
	return TRUE

/obj/item/spacepod_equipment/proc/can_uninstall(mob/user)
	return TRUE

/*
 * Weapon System
 */

/obj/item/spacepod_equipment/weaponry
	slot = SPACEPOD_SLOT_WEAPON
	var/projectile_type
	var/shot_cost = 0
	var/shots_per = 1
	var/fire_sound
	var/fire_delay = 1.5 SECONDS
	var/overlay_icon
	var/overlay_icon_state

/obj/item/spacepod_equipment/weaponry/on_install(obj/spacepod/installed_pod)
	. = ..()
	installed_pod.weapon = src
	installed_pod.update_icon()

/obj/item/spacepod_equipment/weaponry/on_uninstall()
	var/obj/spacepod/old_pod = spacepod
	. = ..()
	if(old_pod?.weapon == src)
		old_pod.weapon = null
		old_pod.update_icon()

/obj/item/spacepod_equipment/weaponry/proc/fire_weapons(atom/target)
	if(!spacepod)
		return
	if(spacepod.next_firetime > world.time)
		to_chat(spacepod.pilot, span_warning("Your weapons are recharging."))
		playsound(src, 'sound/items/weapons/gun/general/dry_fire.ogg', 30, TRUE)
		return
	if(!spacepod.cell || !spacepod.cell.use(shot_cost * SPACEPOD_POWER_SCALE))
		to_chat(spacepod.pilot, span_warning("Insufficient charge to fire the weapons!"))
		playsound(src, 'sound/items/weapons/gun/general/dry_fire.ogg', 30, TRUE)
		return
	spacepod.next_firetime = world.time + fire_delay
	for(var/shot in 1 to shots_per)
		spacepod.fire_projectiles(projectile_type, target)
		playsound(src, fire_sound, 50, TRUE)
		sleep(0.2 SECONDS)

/obj/item/spacepod_equipment/weaponry/disabler
	name = "disabler system"
	desc = "A weak disabler system for space pods, fires disabler beams."
	icon_state = "weapon_taser"
	projectile_type = /obj/projectile/beam/disabler
	shot_cost = 400
	fire_sound = 'sound/items/weapons/taser2.ogg'
	overlay_icon = 'modular_nova/modules/spacepods/icons/overlays_2x2.dmi'
	overlay_icon_state = "pod_weapon_disabler"

/obj/item/spacepod_equipment/weaponry/burst_disabler
	name = "burst disabler system"
	desc = "A weak disabler system for space pods, this one fires 3 at a time."
	icon_state = "weapon_burst_taser"
	projectile_type = /obj/projectile/beam/disabler
	shot_cost = 1200
	shots_per = 3
	fire_sound = 'sound/items/weapons/taser2.ogg'
	fire_delay = 3 SECONDS
	overlay_icon = 'modular_nova/modules/spacepods/icons/overlays_2x2.dmi'
	overlay_icon_state = "pod_weapon_disabler"

/obj/item/spacepod_equipment/weaponry/laser
	name = "laser system"
	desc = "A weak laser system for space pods, fires concentrated bursts of energy."
	icon_state = "weapon_laser"
	projectile_type = /obj/projectile/beam/laser
	shot_cost = 600
	fire_sound = 'sound/items/weapons/laser.ogg'
	overlay_icon = 'modular_nova/modules/spacepods/icons/overlays_2x2.dmi'
	overlay_icon_state = "pod_weapon_laser"

// MINING LASERS
/obj/item/spacepod_equipment/weaponry/basic_pod_ka
	name = "weak kinetic accelerator"
	desc = "A weak kinetic accelerator for space pods, fires bursts of energy that cut through rock."
	icon = 'modular_nova/modules/spacepods/icons/parts.dmi'
	icon_state = "pod_taser"
	projectile_type = /obj/projectile/kinetic/pod
	shot_cost = 300
	fire_delay = 1.4 SECONDS
	fire_sound = 'sound/items/weapons/kinetic_accel.ogg'

/obj/item/spacepod_equipment/weaponry/pod_ka
	name = "kinetic accelerator system"
	desc = "A kinetic accelerator system for space pods, fires bursts of energy that cut through rock."
	icon = 'modular_nova/modules/spacepods/icons/parts.dmi'
	icon_state = "pod_m_laser"
	projectile_type = /obj/projectile/kinetic/pod/regular
	shot_cost = 250
	fire_delay = 1 SECONDS
	fire_sound = 'sound/items/weapons/kinetic_accel.ogg'

/obj/projectile/kinetic/pod
	range = 4

/obj/projectile/kinetic/pod/regular
	damage = 50
	pressure_decrease = 0.5

/obj/item/spacepod_equipment/weaponry/plasma_cutter
	name = "plasma cutter system"
	desc = "A plasma cutter system for space pods. It is capable of expelling concentrated plasma bursts to mine or cut off xeno limbs!"
	icon = 'modular_nova/modules/spacepods/icons/parts.dmi'
	icon_state = "pod_p_cutter"
	projectile_type = /obj/projectile/plasma
	shot_cost = 250
	fire_delay = 1 SECONDS
	fire_sound = 'sound/items/weapons/plasma_cutter.ogg'
	overlay_icon = 'modular_nova/modules/spacepods/icons/overlays_2x2.dmi'
	overlay_icon_state = "pod_weapon_plasma"

/obj/item/spacepod_equipment/weaponry/plasma_cutter/adv
	name = "enhanced plasma cutter system"
	desc = "An enhanced plasma cutter system for space pods. It is capable of expelling concentrated plasma bursts to mine or cut off xeno faces!"
	icon_state = "pod_ap_cutter"
	projectile_type = /obj/projectile/plasma/adv
	shot_cost = 200
	fire_delay = 0.8 SECONDS

/*
 * Cargo System
 */

/obj/item/spacepod_equipment/cargo // this one holds large crates and shit
	name = "pod cargo"
	desc = "You shouldn't be seeing this"
	icon_state = "cargo_blank"
	slot = SPACEPOD_SLOT_CARGO

/obj/item/spacepod_equipment/cargo/large
	name = "spacepod crate storage system"
	desc = "A heavy duty storage system for spacepods. Holds one crate."
	icon_state = "cargo_crate"
	var/obj/storage = null
	var/storage_type = /obj/structure/closet/crate

/obj/item/spacepod_equipment/cargo/large/on_install(obj/spacepod/installed_pod)
	. = ..()
	RegisterSignal(installed_pod, COMSIG_MOUSEDROPPED_ONTO, PROC_REF(spacepod_mousedrop))

/obj/item/spacepod_equipment/cargo/large/on_uninstall()
	if(spacepod)
		UnregisterSignal(spacepod, COMSIG_MOUSEDROPPED_ONTO)
	return ..()

/obj/item/spacepod_equipment/cargo/large/can_uninstall(mob/user)
	if(storage)
		to_chat(user, span_warning("Unload the cargo first!"))
		return FALSE
	return ..()

/obj/spacepod/verb/unload_cargo()
	set name = "Unload Cargo"
	set category = "Spacepod"
	set src = usr.loc

	if(!verb_check())
		return

	var/list/used_key_list = list()
	var/list/cargo_map = list()
	for(var/obj/item/spacepod_equipment/cargo/large/cargo_system in equipment)
		if(!cargo_system.storage)
			continue
		cargo_map[avoid_assoc_duplicate_keys("[cargo_system.name] ([cargo_system.storage.name])", used_key_list)] = cargo_system
	if(!length(cargo_map))
		to_chat(usr, span_warning("There's no cargo to unload."))
		return
	var/selection = tgui_input_list(usr, "Unload which cargo?", "Spacepod", cargo_map)
	var/obj/item/spacepod_equipment/cargo/large/chosen = cargo_map[selection]
	if(!selection || !verb_check() || !chosen || !(chosen in equipment) || !chosen.storage)
		return
	chosen.storage.forceMove(loc)
	chosen.storage = null

/obj/item/spacepod_equipment/cargo/large/proc/spacepod_mousedrop(obj/spacepod/source, atom/movable/dropped, mob/user)
	SIGNAL_HANDLER
	if(user == source.pilot || (user in source.passengers))
		return FALSE
	if(istype(dropped, storage_type) && source.Adjacent(dropped)) // For loading ore boxes
		INVOKE_ASYNC(src, PROC_REF(load_cargo), source, dropped, user)
		return TRUE
	return FALSE

/obj/item/spacepod_equipment/cargo/large/proc/load_cargo(obj/spacepod/source, atom/movable/dropped, mob/user)
	if(storage)
		to_chat(user, span_warning("[source] already has \an [storage]!"))
		return
	to_chat(user, span_notice("You begin loading [dropped] into [source]'s [src]."))
	if(do_after(user, 4 SECONDS, target = source))
		if(storage)
			return
		storage = dropped
		dropped.forceMove(src)
		to_chat(user, span_notice("You load [dropped] into [source]'s [src]!"))
	else
		to_chat(user, span_warning("You fail to load [dropped] into [source]'s [src]."))

/obj/item/spacepod_equipment/cargo/large/ore
	name = "spacepod ore storage system"
	desc = "An ore storage system for spacepods. Scoops up any ore you drive over. Needs to be loaded with an ore box to work."
	icon_state = "cargo_ore"
	storage_type = /obj/structure/ore_box

/obj/item/spacepod_equipment/cargo/large/ore/on_install(obj/spacepod/installed_pod)
	. = ..()
	RegisterSignal(installed_pod, COMSIG_MOVABLE_MOVED, PROC_REF(spacepod_moved))

/obj/item/spacepod_equipment/cargo/large/ore/on_uninstall()
	if(spacepod)
		UnregisterSignal(spacepod, COMSIG_MOVABLE_MOVED)
	return ..()

/obj/item/spacepod_equipment/cargo/large/ore/proc/spacepod_moved(obj/spacepod/source)
	SIGNAL_HANDLER
	if(!storage)
		return
	for(var/turf/pod_turf in source.locs)
		for(var/obj/item/stack/ore/found_ore in pod_turf)
			found_ore.forceMove(storage)

/obj/item/spacepod_equipment/cargo/chair
	name = "passenger seat"
	desc = "A passenger seat for a spacepod."
	icon_state = "sec_cargo_chair"
	var/occupant_mod = 1

/obj/item/spacepod_equipment/cargo/chair/on_install(obj/spacepod/installed_pod)
	. = ..()
	installed_pod.max_passengers += occupant_mod

/obj/item/spacepod_equipment/cargo/chair/on_uninstall()
	if(spacepod)
		spacepod.max_passengers -= occupant_mod
	return ..()

/obj/item/spacepod_equipment/cargo/chair/can_uninstall(mob/user)
	if(length(spacepod.passengers) > (spacepod.max_passengers - occupant_mod))
		to_chat(user, span_warning("You can't remove an occupied seat! Remove the occupant first."))
		return FALSE
	return ..()

/*
 * Misc. System
 */

/obj/item/spacepod_equipment/tracker
	name = "spacepod tracking system"
	desc = "A tracking device for spacepods."
	icon = 'modular_nova/modules/spacepods/icons/parts.dmi'
	icon_state = "pod_locator"

/*
 * Lock System
 */

/obj/item/spacepod_equipment/lock
	name = "pod lock"
	desc = "You shouldn't be seeing this"
	icon_state = "blank"
	slot = SPACEPOD_SLOT_LOCK

/obj/item/spacepod_equipment/lock/on_install(obj/spacepod/installed_pod)
	. = ..()
	RegisterSignal(installed_pod, COMSIG_ATOM_ATTACKBY, PROC_REF(spacepod_attackby))
	installed_pod.lock = src

/obj/item/spacepod_equipment/lock/on_uninstall()
	var/obj/spacepod/old_pod = spacepod
	if(old_pod)
		UnregisterSignal(old_pod, COMSIG_ATOM_ATTACKBY)
		if(old_pod.lock == src)
			old_pod.lock = null
		old_pod.locked = FALSE
	return ..()

/obj/item/spacepod_equipment/lock/proc/spacepod_attackby(obj/spacepod/source, obj/item/weapon, mob/user, list/modifiers, list/attack_modifiers)
	SIGNAL_HANDLER
	return NONE

// Key and Tumbler System
/obj/item/spacepod_equipment/lock/keyed
	name = "spacepod tumbler lock"
	desc = "A locking system to stop podjacking. This version uses a standalone key."
	icon_state = "lock_tumbler"
	var/static/id_source = 0
	var/id = null

/obj/item/spacepod_equipment/lock/keyed/Initialize(mapload)
	. = ..()
	if(id == null)
		id = ++id_source

/obj/item/spacepod_equipment/lock/keyed/spacepod_attackby(obj/spacepod/source, obj/item/weapon, mob/user, list/modifiers, list/attack_modifiers)
	SIGNAL_HANDLER
	if(istype(weapon, /obj/item/spacepod_key))
		var/obj/item/spacepod_key/key = weapon
		if(key.id == id)
			source.lock_pod()
		else
			to_chat(user, span_warning("This is the wrong key!"))
		return COMPONENT_NO_AFTERATTACK
	return NONE

/obj/item/spacepod_equipment/lock/keyed/attackby(obj/item/attacking_item, mob/user, list/modifiers, list/attack_modifiers)
	if(istype(attacking_item, /obj/item/spacepod_key))
		var/obj/item/spacepod_key/key = attacking_item
		if(key.id == null)
			key.id = id
			to_chat(user, span_notice("You grind the blank key to fit the lock."))
		else
			to_chat(user, span_warning("This key is already ground!"))
		return TRUE
	return ..()

/obj/item/spacepod_equipment/lock/keyed/sec
	id = "security spacepod"

// The key
/obj/item/spacepod_key
	name = "spacepod key"
	desc = "A key for a spacepod lock."
	icon = 'modular_nova/modules/spacepods/icons/equipment.dmi'
	icon_state = "podkey"
	w_class = WEIGHT_CLASS_TINY
	var/id = null

/obj/item/spacepod_key/sec
	name = "security spacepod key"
	desc = "Unlocks the security spacepod. Probably best kept out of assistant hands."
	id = "security spacepod"

/obj/item/lock_buster
	name = "pod lock buster"
	desc = "Destroys a podlock in mere seconds once applied. Warranty void if used."
	icon = 'modular_nova/modules/spacepods/icons/equipment.dmi'
	icon_state = "lock_buster_off"
	var/on = FALSE

/obj/item/lock_buster/attack_self(mob/user, modifiers)
	on = !on
	icon_state = on ? "lock_buster_on" : "lock_buster_off"
	to_chat(user, span_notice("You turn [src] [on ? "on" : "off"]."))
	return TRUE
