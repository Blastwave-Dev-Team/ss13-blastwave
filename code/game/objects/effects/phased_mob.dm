...
/obj/effect/dummy/phased_mob/proc/phased_check(mob/living/user, direction)
	...
	if(newloc.turf_flags & NOJAUNT)
		...
		return
	if(destination_area.area_flags & NOTELEPORT || SSmapping.level_trait(newloc.z, ZTRAIT_NOPHASE) || is_teleport_jammed(newloc)) // BLASTWAVE EDIT CHANGE - BLASTWAVE_BLUESPACE - ORIGINAL: if(destination_area.area_flags & NOTELEPORT || SSmapping.level_trait(newloc.z, ZTRAIT_NOPHASE))
